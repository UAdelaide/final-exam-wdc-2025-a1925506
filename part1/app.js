...
app.get('/api/walkrequests/open', async (req, res) => {
    try {
      const [walkersummaries] = await db.execute('SELECT Users.username, COUNT(WalkRatings.rating WHERE WalkRatings.walker_id = 2) AS total_ratings, AVG(WalkRatings.rating WHERE WalkRatings.walker_id = 2), COUNT(WalkRatings.rating_id WHERE WalkRatings.walker_id = 2);');
