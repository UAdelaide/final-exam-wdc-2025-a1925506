...
app.get('/api/walkers/summary', async (req, res) => {
    try {
      const [walkersummaries] = await db.execute('SELECT Users.username, COUNT(WalkRatings.rating) AS total_ratings, TRUNCATE (AVG(WalkRatings.rating), 1) AS average_rating, COUNT(WalkRatings.rating_id) AS completed_walks FROM WalkRatings INNER JOIN Users ON WalkRatings.walker_id = Users.user_id WHERE WalkRatings.walker_id = 2;');
